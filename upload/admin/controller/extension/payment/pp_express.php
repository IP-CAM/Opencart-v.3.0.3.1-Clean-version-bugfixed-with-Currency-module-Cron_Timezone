...
class ControllerExtensionPaymentPpExpress extends Controller {
	...
	protected $opencart_connect_url = 'https://www.opencart.com/index.php?route=external/paypal_auth/connect';
	protected $opencart_retrieve_url = 'https://www.opencart.com/index.php?route=external/paypal_auth/retrieve';
	public function index() {
		...
		$data['paypal_zones'] = $this->model_extension_payment_pp_express->getUnmatchedZoneCodes();
	...
	protected function validate() {
		...
		if (isset($this->request->post['payment_pp_express_test']) && $this->request->post['payment_pp_express_test']) {
			if (!isset($this->request->post['payment_pp_express_sandbox_username']) || !$this->request->post['payment_pp_express_sandbox_username']) {
				$this->error['sandbox_username'] = $this->language->get('error_sandbox_username');
			}
			if (!isset($this->request->post['payment_pp_express_sandbox_password']) || !$this->request->post['payment_pp_express_sandbox_password']) {
				$this->error['sandbox_password'] = $this->language->get('error_sandbox_password');
			}
			if (!isset($this->request->post['payment_pp_express_sandbox_signature']) || !$this->request->post['payment_pp_express_sandbox_signature']) {
				$this->error['sandbox_signature'] = $this->language->get('error_sandbox_signature');
			}
		} else {
			if (!isset($this->request->post['payment_pp_express_username']) || !$this->request->post['payment_pp_express_username']) {
				$this->error['username'] = $this->language->get('error_username');
			}
			if (!isset($this->request->post['payment_pp_express_password']) || !$this->request->post['payment_pp_express_password']) {
				$this->error['password'] = $this->language->get('error_password');
			}
			if (!isset($this->request->post['payment_pp_express_signature']) || !$this->request->post['payment_pp_express_signature']) {
	...
	public function capture() {
		...
		if (!$json) {
			...
			if ($paypal_info) {
				...
				if (isset($this->request->post['complete']) && (int)$this->request->post['complete'] == 1) {
					$status = 'Complete';
				} else {
					$status = 'NotComplete';
				}
				...
				if (isset($response['ACK']) && ($response['ACK'] != 'Failure') && ($response['ACK'] != 'FailureWithWarning')) {
					...
					if (isset($this->request->post['complete']) && $this->request->post['complete'] == 1 || $json['remaining'] == 0.00) {
	...
	public function doRefund() {
		...
		if (isset($this->request->post['transaction_id']) && isset($this->request->post['refund_full'])) {
			...
			$this->load->language('extension/payment/pp_express_refund');
			if (isset($this->request->post['refund_full']) && isset($this->request->post['amount'])) {
				if ($this->request->post['refund_full'] == 0 && $this->request->post['amount'] == 0) {
					...
					if ($paypal_order) {
						...
						$call_data['NOTE'] = (isset($this->request->post['refund_message']) ? urlencode($this->request->post['refund_message']) : '');
						...
						if (isset($this->request->post['refund_full'])) {
							if ($this->request->post['refund_full'] == 1) {
								$call_data['REFUNDTYPE'] = 'Full';
							} elseif (isset($this->request->post['currency_code'])) {
						...
						$transaction = array(
							...
							'note' 						=> (isset($this->request->post['refund_message']) ? $this->request->post['refund_message'] : ''),
						...
						if (!$result) {
							...
							if (isset($this->request->post['amount_original']) && $result['TOTALREFUNDEDAMOUNT'] == $this->request->post['amount_original']) {
	...
	public function void() {
		...
		if ($paypal_info) {
			...
			$response = $this->model_extension_payment_pp_express->call($request);
			if (isset($response['ACK']) && ((string)$response['ACK'] != 'Failure') && ((string)$response['ACK'] != 'FailureWithWarning')) {
				$transaction = array(
					'paypal_order_id'       => $paypal_info['paypal_order_id'],
					'transaction_id'        => '',
					'parent_id'             => $paypal_info['authorization_id'],
	...
	public function doSearch() {
		...
		if (isset($this->request->post['date_start'])) {
			...
			if ($result['ACK'] != 'Failure' && $result['ACK'] != 'FailureWithWarning' && $result['ACK'] != 'Warning') {
				...
				$json['result'] = $this->formatRows($result);
			} else {
				$json['error'] = true;				
				$json['error_msg'] = $result['L_LONGMESSAGE0'];
			}
			$this->response->addHeader('Content-Type: application/json');
			$this->response->setOutput(json_encode($json));
		} else {
			...
			$json['error_msg'] = $this->language->get('error_start_date');
